...
const reverseSequelizeColType = function(col, prefix = 'Sequelize.') 
{
    ...
    switch (attrName) {
        ...
        case DataTypes.ARRAY.key:
          ...
          let regex = /(\w+)\[\]/;
          let match = _type.match(regex);
          let arrayType = match.length >= 1 ? prefix + match[1] : prefix + 'STRING';
...
const sortDownActions = function(downActions, upActions)
{
    let sorted = [];
    _.forEachRight(upActions,
        upAction => sorted.push(_.find(downActions, downAction => downAction.tableName == upAction.tableName)))
    return sorted;
}
...
module.exports = { writeMigration, getMigration, sortActions, sortDownActions, parseDifference, reverseModels, executeMigration };
